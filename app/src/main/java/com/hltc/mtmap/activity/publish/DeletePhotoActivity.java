...
public class DeletePhotoActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        for (int i = 0; i < PhotoHelper.bitmaps.size(); i++) {
            bmp.add(PhotoHelper.bitmaps.get(i));
        }
        for (int i = 0; i < PhotoHelper.addresses.size(); i++) {
            drr.add(PhotoHelper.addresses.get(i));
        }
        max = PhotoHelper.max;
        ...
        photo_bt_del.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                if (listViews.size() == 1) {
                    PhotoHelper.bitmaps.clear();
                    PhotoHelper.addresses.clear();
                    PhotoHelper.max = 0;
                    FileUtils.deleteDir();
                    finish();
                } else {
        ...
        photo_bt_enter.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                PhotoHelper.bitmaps = bmp;
                PhotoHelper.addresses = drr;
                PhotoHelper.max = max;
